...
export default function StrategyAutoTradeModal({ strategyId, strategyName, symbol, onClose, onSaved }) {
  ...
  if (!cfg) return (
    <div className="sat-backdrop">
      <div className="sat-modal"><div className="sat-loading">Lädt...</div></div>
    </div>
  );
  ...
  return (
    <div className="sat-backdrop" onClick={onClose}>
      <div className="sat-modal" onClick={e => e.stopPropagation()} data-testid="strategy-autotrade-modal">

        {/* Header */}
        <div className="sat-header">
          <span className="sat-title">
            ⚡ Auto-Trade · <b>{strategyName || strategyId}</b>
            {coinShort && (
              <span className="sat-coin-badge" style={{
                marginLeft: 8,
                background: '#f59e0b22',
                color: '#f59e0b',
          ...
          <button className="sat-close" onClick={onClose}>✕</button>
        </div>
        ...
        <div className="sat-footer">
          <button className="sat-cancel" onClick={onClose}>Abbrechen</button>
          <button
            className="sat-save"
            onClick={save}
            disabled={saving}
