...
const StrategyBuilder = ({ strategies, enabledIds, onClose, onChanged }) => {
  ...
  const create = async () => {
    ...
    const res = await fetch(`${API_URL}/api/strategies/custom`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify(def),
    });
    if (res.ok) {
  ...
  const deleteCustom = async (id) => {
    if (!isAdmin()) { toast.error('Admin-Login erforderlich'); return; }
    const res = await fetch(`${API_URL}/api/strategies/custom/${id}`, { method: 'DELETE', headers: { ...authHeaders() } });
    if (res.ok) { toast.success('Strategie gelöscht'); onChanged && onChanged(); }
