...
const AutoTradeModal = ({ symbol, onClose }) => {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await fetch(`${API_URL}/api/autotrade/config`, {
      ...
      if (res.ok) toast.success(`Auto-Trade ${symbol.replace('USDT', '')} gespeichert`);
  ...
  const closeTrade = async (id) => {
    await fetch(`${API_URL}/api/autotrade/close/${id}`, { method: 'POST' });
