...
def _round_step(value: float, step: float) -> str:
    ...
    # normalize: drop useless trailing zeros but keep required precision
    s = format(quant.normalize(), "f")
...
class BitunixTradeClient:
    ...
    async def load_trading_pairs(self) -> None:
        ...
        for row in data:
            ...
            try:
                meta[sym] = {
                    "qty_step": float(row.get("basePrecision") or row.get("minTradeVolume") or 0) or 0.0,
                    "price_tick": float(row.get("quotePrecision") or row.get("pricePrecision") or 0) or 0.0,
    ...
    def _fmt_price(self, bitunix_symbol: str, price: float) -> str:
        m = self._pairs_meta.get(bitunix_symbol) or {}
    ...
    async def place_order(self, symbol, side, qty, order_type="MARKET", price=None,
        ...
        if tp_price:
            body.update({"tpPrice": self._fmt_price(b_symbol, tp_price),
                         "tpStopType": "MARK_PRICE", "tpOrderType": "MARKET"})
        if sl_price:
            body.update({"slPrice": self._fmt_price(b_symbol, sl_price),
